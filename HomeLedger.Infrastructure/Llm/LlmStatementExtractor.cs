...
public class LlmStatementExtractor : ILlmStatementExtractor
{
    ...
    public LlmStatementExtractor(HttpClient http, IOptionsMonitor<LlmSettings> settings, ILogger<LlmStatementExtractor> logger)
    {
        _http = http;
        _settings = settings;
        _logger = logger;
    ...
    public bool IsEnabled => _settings.CurrentValue.IsStatementImportEffective();
        ...
        CancellationToken ct = default)
    {
        ...
        var responseText = await LlmVisionHelper.CompleteAsync(_http, _settings.CurrentValue, ExtractionPrompt, pages, _logger, ct);
