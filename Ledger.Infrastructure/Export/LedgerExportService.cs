...
using Ledger.Core.Export;
using Ledger.Core.Utilities;
using Ledger.Infrastructure.Data;
using Microsoft.EntityFrameworkCore;

namespace Ledger.Infrastructure.Export;
...
public interface ILedgerExportService
{
    ...
    Task<LedgerImportResult> ImportCsvAsync(Stream csvStream, CancellationToken ct = default);
    string[] ReadCsvHeaders(Stream csvStream);
}

public class LedgerExportService : ILedgerExportService
{
    private readonly LedgerDbContext _db;

    public LedgerExportService(LedgerDbContext db) => _db = db;
    ...
    public async Task<byte[]> ExportCsvAsync(CancellationToken ct = default)
    {
        ...
        foreach (var header in LedgerExportFormat.Headers)
            csv.WriteField(header);
        await csv.NextRecordAsync();

        WriteRow(csv, LedgerExportFormat.RecordMeta, notes: LedgerExportFormat.Version,
            createdAt: DateTime.UtcNow.ToString("O"));

        foreach (var entity in entities)
        {
            WriteRow(csv, LedgerExportFormat.RecordEntity,
        ...
        {
            WriteRow(csv, LedgerExportFormat.RecordAccount,
        ...
        {
            WriteRow(csv, LedgerExportFormat.RecordCategoryGroup,
        ...
        {
            WriteRow(csv, LedgerExportFormat.RecordCategory,
        ...
        {
            WriteRow(csv, LedgerExportFormat.RecordBudget,
        ...
        {
            WriteRow(csv, LedgerExportFormat.RecordTransaction,
    ...
    public async Task<LedgerImportResult> ImportCsvAsync(Stream csvStream, CancellationToken ct = default)
    {
        ...
        foreach (var row in rows.Where(r => r.RecordType == LedgerExportFormat.RecordEntity))
        {
        ...
        foreach (var row in rows.Where(r => r.RecordType == LedgerExportFormat.RecordAccount))
        {
        ...
        foreach (var row in rows.Where(r => r.RecordType == LedgerExportFormat.RecordCategoryGroup))
        {
        ...
        foreach (var row in rows.Where(r => r.RecordType == LedgerExportFormat.RecordCategory))
        {
            var entityId = await ResolveScopeEntityIdAsync(row.EntityName, entityMap, ct);
            var groupName = Require(row.CategoryGroupName, "category group name");
        ...
        foreach (var row in rows.Where(r => r.RecordType == LedgerExportFormat.RecordBudget))
        {
            var entityId = await ResolveScopeEntityIdAsync(row.EntityName, entityMap, ct);
            var categoryName = Require(row.CategoryName, "category name");
        ...
        foreach (var row in rows.Where(r => r.RecordType == LedgerExportFormat.RecordTransaction))
        {
            var entityName = Require(row.EntityName, "entity name");
            var categoryName = Require(row.CategoryName, "category name");
    ...
    {
        var meta = rows.FirstOrDefault(r => r.RecordType == LedgerExportFormat.RecordMeta);
        if (meta is null || !string.Equals(meta.Notes, LedgerExportFormat.Version, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Unsupported or missing ledger export format. Expected meta row with version '{LedgerExportFormat.Version}'.");
    }
    ...
    private static string? FormatDate(DateOnly? date) => date is null ? null : LedgerFormats.FormatDate(date.Value);
